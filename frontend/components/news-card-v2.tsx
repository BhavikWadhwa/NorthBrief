...
const fallbackByCategory: Record<string, string> = {
  local: "/images/fallback/local.svg",
  canada: "/images/fallback/canada.svg",
  world: "/images/fallback/world.svg",
  global: "/images/fallback/world.svg",
  finance: "/images/fallback/finance.svg",
  global_finance: "/images/fallback/finance.svg",
  ai: "/images/fallback/ai.svg",
  humanitarian: "/images/fallback/humanitarian.svg"
};
...
export function NewsCard({ item, onBookmark }: NewsCardProps) {
  ...
  const fallbackImage =
    fallbackByCategory[item.category.toLowerCase().replaceAll(" ", "_")] ?? "/images/fallback/generic.svg";
  const [hasImageError, setHasImageError] = useState(false);
  const displayImage = !hasImageError && item.image_url ? item.image_url : fallbackImage;
  ...
  return (
    <article className="animate-fade-up rounded-2xl border border-zinc-200 bg-white/90 p-5 shadow-sm transition hover:shadow-md dark:border-zinc-800 dark:bg-zinc-900/80">
      <div className="relative mb-4 h-44 overflow-hidden rounded-xl border border-zinc-200 dark:border-zinc-800">
        <Image
          src={displayImage}
          alt={item.headline}
          fill
